...
data1 <- data %>%
  ...
  select(
    TF_name, PWM,
    roc_auc_test_M_PWM_mono, roc_auc_test_M,
    pr_auc_test_M_PWM_mono, pr_auc_test_M,
    Count, Seq_count
  )
data2 <- data1 %>%
  group_by(TF_name) %>%
...
data <- data3 %>%
  mutate(
    ROC_delta_M = roc_auc_test_M - roc_auc_test_M_PWM_mono,  # compare mono+di model vs mono baseline
    PR_delta_M  = pr_auc_test_M  - pr_auc_test_M_PWM_mono    # compare mono+di model vs mono baseline
  ) %>% 
  group_by(TF_name) %>% 
  summarise(
...
data = data %>%
  ...
  select(c("TF_name",
           "roc_auc_test_M_PWM_mono", "roc_auc_test_M",
           "pr_auc_test_M_PWM_mono", "pr_auc_test_M", "Count", "Seq_count", "PWM"))

a = data %>%
  arrange(desc(Seq_count)) %>%
  ggplot(aes(x=roc_auc_test_M_PWM_mono, y=roc_auc_test_M, color=PWM)) + # 
...
b = data %>%
  arrange(desc(Seq_count))  %>%
  ggplot(aes(x=pr_auc_test_M_PWM_mono, y=pr_auc_test_M, color=PWM)) + # 
  geom_point(alpha=0.5, shape=20, size=3)+
  theme_minimal(base_size = 21) +
  theme(legend.position="none",
        axis.text.x=element_text(angle=90, vjust = 0.5))+
...
Cairo(file='Figure_2_20012026_mono_di_HUMAN_MOUSE_RandomForestClassifier.pdf', type="pdf", width=310, height=185, units="mm")
image
dev.off()
