...
**ArChIPelago** (**Ar**rangement of multiple **ChIP**-seq and **H**igh-throughput analysis using machine **l**earning for **p**rediction of transcription factor binding sites) is a machine learning framework that combines hundreds of position weight matrices (PWMs) into Random Forest ensemble classifiers to predict transcription factor binding sites from DNA sequence alone.

ArChIPelago was trained and evaluated on 36 human transcription factors using ChIP-seq data from the GTRD database and PWMs from the HOCOMOCO v11 collection. Models trained on human data were cross-validated on human held-out chromosomes and independently validated on mouse orthologs, demonstrating strong cross-species generalization.

This repository contains the full analysis pipeline, pre-trained models, and code to reproduce all results from *Kravchenko et al., (2026)*.
...
2. Scan sequences with hundreds of HOCOMOCO v11 PWMs (mono- and dinucleotide) using SARUS
3. Build feature matrices from best-hit PWM scores
4. Train `RandomForestClassifier` ensembles (one per TF)
5. Evaluate with ROC-AUC and PR-AUC on held-out human and mouse test sets
6. Compare against single-PWM baselines, SLIM, and other classifiers
...
- 36 transcription factors modeled
- Up to 198 mono + 99 di PWMs per TF
- Median ROC-AUC **0.95** on human test set
- Cross-species validation on mouse orthologs
- Outperforms single best PWM for all 36 TFs
...
│   ├── run_simulation.py            # End-to-end pipeline smoke test
...
│   ├── Figure 2 and S1/             # Model comparison (boxplots)
│   ├── Figure 3 and S3/             # PWM vs ensemble (scatter + delta)
│   └── Figure 4/                    # SLIM/diChIPMunk comparison
│
├── ArChIPelago-TFBS-finder/         # Standalone scanning tool (git submodule)
├── sarus/                           # SARUS PWM scanner (git submodule)
...
├── Table_2.xlsx                     # TF metadata (experiments, peaks, GC%)
...
| Step | What it does |
|------|-------------|
| Load sequences | Positive (ChIP-seq peaks) and negative (background) FASTA |
| PWM scanning | SARUS scan with HOCOMOCO v11 mono + di PWMs |
...
| Evaluate | ROC-AUC, PR-AUC on held-out test chromosomes |
...
- **Java 8+** (for SARUS PWM scanning; OpenJDK 8 is bundled in `Slim/jdk8u232-b09/`)
...
> **Note on R:** The environment includes R 4.3.1 and rpy2 3.5.11 for PRROC-based ROC/PR curve computation (used in the publication). If R is not needed, the sklearn-based scorers in `archipielago/scoring.py` provide equivalent functionality.
...
| 1 | PWM scanning | FASTA + HOCOMOCO PWMs | Per-TF SARUS feature matrices |
| 2 | Model training | Feature matrices | Trained RF/SLIM models, ROC/PR scores |
| 3 | BiasAway QC | Negative sequences | GC-content quality control plots |
| 4 | Figures | Model scores | Publication figures |

**Train/test split:** Training uses chromosomes 2–7, 9–20; testing uses chromosomes 1, 8, 21 (human) and chromosomes 1, 8, 19 (mouse). This chromosome-level split ensures no data leakage between train and test sets.
...
|--------|--------|-------------|
| Fig. 2 | `Figure 2 and S1/Figure_2_21012026_H_H.R` | Model comparison boxplots (human→human) |
| Fig. S1 | `Figure 2 and S1/Figure_2_210120267_H_M.R` | Model comparison boxplots (human→mouse) |
| Fig. 3 | `Figure 3 and S3/Figure_3_H_H_20012026.R` | PWM vs RF ensemble scatter + improvement (H→H) |
| Fig. S3 | `Figure 3 and S3/Figure_S3_H_M_20012026.R` | PWM vs RF ensemble scatter + improvement (H→M) |
| Fig. 4 | `Figure 4/Figure_4_H_H_7012026.R` | SLIM/diChIPMunk comparison (delta, H→H) |
| Fig. S4 | `Figure 4/Figure_S4_H_M_7012026.R` | SLIM/diChIPMunk comparison (delta, H→M) |
...
The R scripts read from `HUMAN_MOUSE_total_100k.csv` (generated by Notebook 4). All mono+di comparisons use the mononucleotide single-PWM baseline, consistent across all figures.
...
- **Python 3.8.18**, scikit-learn 1.3.0, numpy 1.24.4, pandas 2.0.3
...
Kravchenko et al., (2026). *ArChIPelago: PWM-ensemble Random Forest classifiers for transcription factor binding site prediction.* (manuscript in preparation)
